...
export default {
  ...
  data: function() {
    return {
      first_left: 0,
      second_left: this.swiper_data.length * this.item_width,
      css_time: 3,
      auto: ""
    };
  },
  ...
  methods: {
    ...
    doneThis: function() {
      let self = this;
      if (self.first_left === -self.swiper_data.length * self.item_width) {
        self.first_left = self.swiper_data.length * self.item_width;
        self.css_time = 0;
      } else if (
        self.second_left ===
        -self.swiper_data.length * self.item_width
      ) {
        self.second_left = self.swiper_data.length * self.item_width;
        self.css_time = 0;
      } else {
        self.first_left = self.first_left - self.item_width;
        self.second_left = self.second_left - self.item_width;
        self.css_time = 3;
      }
    },
